...
rule = do
    ...
    pure $ case c of
        ...
        w@(When _ _      ) -> Rule id $ Constraints $ argConstrs ++ [w]
...
func = do
    ...
    symbol $ choice $ map string ["is", "are"]
    constr <- symbol constraint
    symbol $ string "."

    pure $ case constr of
        Constraint cid -> Func id (Constraints argConstrs) $ Expr cid
        w@(When _ _) -> Func id (Constraints (argConstrs ++ [makeResult w])) $ Expr $ Comp [funcResultVar]
...
constraint = do
    ...
    whenBranches <- sepEndBy (try when) $ symbol $ string ";"
    defBranch <- try (optionMaybe defaultBranch) >>= \case
                    Nothing -> pure []
                    Just x -> pure [x]

    pure $ Constraints $ constraints ++ whenBranches ++ defBranch
    where
        ignoreWords = ["when", "then", "default"]
        idParsers = do
            id <- try (enkiId "" ignoreWords <|> baseEnkiId "" ignoreWords)
            case id of
                Comp [] -> parserZero
                _       -> pure id
        sep = symbol $ string ","
...
defaultBranch  :: Parser Constraint
defaultBranch = do
