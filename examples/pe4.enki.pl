...
main(Argv) :-
    answer_with_digits(3,Temp78),
    the(Temp78,Temp79),
    as_text(Temp79,Temp80),
    display(Temp80).
% EnkiString
display(X) :-
    writeln(X).
...
% FuncType EnkiInt (TypeName [Named "list",Any "T823"])
digit_numbers(N,Temp66) :-
    Temp64 #= (N - 1),
    Temp65 #= (10 ^ Temp64),
    Temp62 #= (10 ^ N),
    Temp63 #= (Temp62 - 1),
    range_to(Temp65,Temp63,Temp66).

% FuncType (TypeName [Named "list",Any "T867"]) (TypeName [Named "list",Any "T858"])
product_pairs_in(List,AUTOGENERATEDFUNCTIONRESULT) :-
    (
            List = cons(pair_and(A,B),T)
            ->
            Temp67 #= (A * B),
            product_pairs_in(T,Temp68),
            prepend_to(Temp67,Temp68,Temp69),
            AUTOGENERATEDFUNCTIONRESULT = Temp69
        ;
            AUTOGENERATEDFUNCTIONRESULT = empty()
    ).

% FuncType (TypeName [Named "list",EnkiInt]) (TypeName [Named "list",Any "T892"])
palindromic_numbers_in(List,AUTOGENERATEDFUNCTIONRESULT) :-
    (
            List = cons(H,T),
            palindromic_number(H)
            ->
            palindromic_numbers_in(T,Temp70),
            prepend_to(H,Temp70,Temp71),
            AUTOGENERATEDFUNCTIONRESULT = Temp71
        ;
        (
                List = cons(H,T)
                ->
                palindromic_numbers_in(T,Temp72),
...
% FuncType EnkiInt EnkiInt
answer_with_digits(N,Temp77) :-
    digit_numbers(N,Temp73),
    Nums = Temp73,
    pairs_of_and(Nums,Nums,Temp74),
    product_pairs_in(Temp74,Temp75),
    palindromic_numbers_in(Temp75,Temp76),
    maximum_of(Temp76,Temp77).
