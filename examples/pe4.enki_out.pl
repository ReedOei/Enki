...
main(Argv) :-
    answer_with_digits(3,Temp105),
    the(Temp105,Temp106),
    as_text(Temp106,Temp107),
    display(Temp107).
% EnkiString
display(X) :-
    writeln(X).
...
% FuncType EnkiInt (TypeName [Named "list",Any "T1370"])
digit_numbers(N,Temp93) :-
    Temp91 #= (N - 1),
    Temp92 #= (10 ^ Temp91),
    Temp89 #= (10 ^ N),
    Temp90 #= (Temp89 - 1),
    range_to(Temp92,Temp90,Temp93).

% FuncType (TypeName [Named "list",Any "T1414"]) (TypeName [Named "list",Any "T1405"])
product_pairs_in(List,AUTOGENERATEDFUNCTIONRESULT) :-
    (
            List = cons(pair_and(A,B),T)
            ->
            Temp94 #= (A * B),
            product_pairs_in(T,Temp95),
            prepend_to(Temp94,Temp95,Temp96),
            AUTOGENERATEDFUNCTIONRESULT = Temp96
        ;
            AUTOGENERATEDFUNCTIONRESULT = empty()
    ).

% FuncType (TypeName [Named "list",EnkiInt]) (TypeName [Named "list",Any "T1439"])
palindromic_numbers_in(List,AUTOGENERATEDFUNCTIONRESULT) :-
    (
            List = cons(H,T),
            palindromic_number(H)
            ->
            palindromic_numbers_in(T,Temp97),
            prepend_to(H,Temp97,Temp98),
            AUTOGENERATEDFUNCTIONRESULT = Temp98
        ;
        (
                List = cons(H,T)
                ->
                palindromic_numbers_in(T,Temp99),
                AUTOGENERATEDFUNCTIONRESULT = Temp99
            ;
                AUTOGENERATEDFUNCTIONRESULT = empty()
        )
    ).
...
% FuncType EnkiInt EnkiInt
answer_with_digits(N,Temp104) :-
    digit_numbers(N,Temp100),
    Nums = Temp100,
    pairs_of_and(Nums,Nums,Temp101),
    product_pairs_in(Temp101,Temp102),
    palindromic_numbers_in(Temp102,Temp103),
    maximum_of(Temp103,Temp104).