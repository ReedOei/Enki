...
main(Argv) :-
    integers_from_to(1,20,Temp127),
    the(Temp127,Temp126),
    list_lcm_of(Temp126,Temp125),
    the(Temp125,Temp124),
...
lcm(A,B,Temp120) :-
    Temp118 #= (A * B),
    gcd_of_and(A,B,Temp119),
    Temp120 #= (Temp118 div Temp119).
...
list_lcm_of(List,AUTOGENERATEDFUNCTIONRESULT) :-
    (
            List = cons(H,T)
            ->
            list_lcm_of(T,Temp121),
            lcm(H,Temp121,AUTOGENERATEDFUNCTIONRESULT),
            AUTOGENERATEDFUNCTIONRESULT = AUTOGENERATEDFUNCTIONRESULT
        ;
            AUTOGENERATEDFUNCTIONRESULT = 1
    ).