...
main(Argv) :-
    fib_seq_up_to(4000000,Temp123),
    multiples_of_in(2,Temp123,Temp122),
    sum_of(Temp122,Temp121),
    as_text(Temp121,Temp120),
    display(Temp120).
% EnkiString
display(X) :-
    writeln(X).
...
% FuncType EnkiInt (FuncType EnkiInt (FuncType EnkiInt (TypeName [Named "list",EnkiInt])))
fib_nums(A,B,Limit,AUTOGENERATEDFUNCTIONRESULT) :-
    (
        ...
        ;
            Temp117 #= (A + B),
            fib_nums(B,Temp117,Limit,Temp116),
            prepend_to(A,Temp116,AUTOGENERATEDFUNCTIONRESULT),
            AUTOGENERATEDFUNCTIONRESULT = AUTOGENERATEDFUNCTIONRESULT
    ).

% FuncType EnkiInt (TypeName [Named "list",EnkiInt])
fib_seq_up_to(N,Temp118) :-
    fib_nums(0,1,N,Temp118).