...
gcd_of_and(A,B,AUTOGENERATEDFUNCTIONRESULT) :-
    ...
    M #>= 0,
    M #< B,
    gcd_of_and(B,M,Temp23),
    AUTOGENERATEDFUNCTIONRESULT = Temp23
    ).

lcm_of_and(A,B,Temp0) :-
    Temp1 #= A * B,
    gcd_of_and(A,B,Temp5),
    Temp0 #= Temp1 div Temp5.

lcm_of(List,AUTOGENERATEDFUNCTIONRESULT) :-
    (
    List = cons(H,T) -> 
    lcm_of(T,Temp15),
    lcm_of_and(H,Temp15,Temp11),
    AUTOGENERATEDFUNCTIONRESULT = Temp11
    );
    (
