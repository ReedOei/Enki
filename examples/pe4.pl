...
digit_numbers(N,Temp0) :-
    Temp4 #= N - 1,
    Temp2 #= 10 ^ Temp4,
    Temp11 #= 10 ^ N,
    Temp10 #= Temp11 - 1,
    range_to(Temp2,Temp10,Temp0).

product_of_pairs_in(List,AUTOGENERATEDFUNCTIONRESULT) :-
    (
    List = cons(pair_and(A,B),T) -> 
    Temp19 #= A * B,
    product_of_pairs_in(T,Temp24),
    AUTOGENERATEDFUNCTIONRESULT = cons(Temp19,Temp24)
    );
    (
    AUTOGENERATEDFUNCTIONRESULT = empty
    ).

palindrome_numbers_in(List,AUTOGENERATEDFUNCTIONRESULT) :-
    (
    List = cons(H,T),
    palindrome_number(H) -> 
    palindrome_numbers_in(T,Temp19),
    AUTOGENERATEDFUNCTIONRESULT = cons(H,Temp19)
    );
    (
    List = cons(H,T) -> 
    palindrome_numbers_in(T,Temp37),
    AUTOGENERATEDFUNCTIONRESULT = Temp37
    );
    (
    AUTOGENERATEDFUNCTIONRESULT = empty
    ).

answer_with_digits(N,Temp7) :-
    digit_numbers(N,Temp2),
    Nums = Temp2,
    pairs_of_and(Nums,Nums,Temp13),
    product_of_pairs_in(Temp13,Temp11),
    palindrome_numbers_in(Temp11,Temp9),
    maximum_of(Temp9,Temp7).
