...
main(Argv) :-
    fib_seq_up_to(4000000,Temp91),
    multiples_of_in(2,Temp91,Temp92),
    sum_of(Temp92,Temp93),
    as_text(Temp93,Temp94),
    display(Temp94).
% EnkiString
display(X) :-
    writeln(X).
...
compare_keys_to(M1,M2,Temp65) :-
    M1 = pipe_dash_gt_(K1,V1),
    M2 = pipe_dash_gt_(K2,V2),
...
compare_values_to(M1,M2,Temp66) :-
    M1 = pipe_dash_gt_(K1,V1),
    M2 = pipe_dash_gt_(K2,V2),
...
% FuncType EnkiInt (FuncType EnkiInt (FuncType EnkiInt (TypeName [Named "list",Any "T1354"])))
fib_nums(A,B,Limit,AUTOGENERATEDFUNCTIONRESULT) :-
    (
        ...
        ;
            Temp87 #= (A + B),
            fib_nums(B,Temp87,Limit,Temp88),
            prepend_to(A,Temp88,Temp89),
            AUTOGENERATEDFUNCTIONRESULT = Temp89
    ).

% FuncType EnkiInt (TypeName [Named "list",Any "T1375"])
fib_seq_up_to(N,Temp90) :-
    fib_nums(0,1,N,Temp90).